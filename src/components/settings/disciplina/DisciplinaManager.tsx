...
import { deleteDisciplinaAction } from "~/actions/settings/disciplines.actions";
...
export default function DisciplinaManager({ initialDisciplinas }: DisciplinaManagerProps) {
    ...
    const handleDelete = async (disciplina: Disciplina) => {
        ...
        if (confirmed) {
            startTransition(async () => {
                const response = await deleteDisciplinaAction(disciplina.id);
                if (response.success) {
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between px-2">
                    ...
                    <span className="px-3 py-1 rounded-full bg-slate-100 text-slate-500 text-xs font-bold uppercase tracking-widest">
                        {initialDisciplinas.length} cadastradas
                    </span>
                </div>

                {initialDisciplinas.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 bg-slate-50 rounded-[3rem] border-2 border-dashed border-slate-200">
                        ...
                        <p className="text-slate-500 font-medium text-sm">Nenhuma disciplina cadastrada</p>
                    </div>
                ) : (
                                ...
                                <div className="flex items-center justify-between gap-4">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="flex flex-col">
                                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Disciplina</span>
